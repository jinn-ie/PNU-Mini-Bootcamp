...
class AuthService:
    ...
    def get_hashed_pwd(self, pwd: str) -> str:
        ...
        salt = bcrypt.gensalt() # 시드값...?
        return bcrypt.hashpw(encoded_pwd, salt)
    
    # 2. Password 검증
    # 로그인 -> password 검증 -> DB 암호화된 password
    # == 클라이언트입력 암호화된 password
    ...
    # (1)번의 password 암호화 함수를 이용하여 암호화된 패스워드를 User.pwd에 저장
    # DB에 User를 넣는다
    ...
    def get_user_by_id(self, db: Session, login_id: str) -> User | None:
    ...
    # DB에 저장된 패스워드와 일치하는지 (2)번 함수로 검사
    def signin(self, db: Session, login_id: str, pwd: str) -> User | None:
        dbUser = self.get_user_by_id(db, login_id)
        if not dbUser:
            return None
